    ...
    <div class="content">
    	...
    	<c:if test="${userRoleType != null && userRoleType != 0 }">
    	  <c:if test="${userRoleType == 1}">
    		...
    		<ul>
	    		<li>
	    			<input type="hidden"  name="employee.orgId" id="orgId" value="${employee.orgId}"/>
					<c:choose>
	    				<c:when test="${userRoleType == 2 }">
			    			...
			    			<select id="org3" class="input-select" onchange="initValue('org3');">
			    				<c:forEach items="${orgList}" var="organization">
			    					<option value="${organization.id}" <c:if test="${organization.id == employee.orgId}">selected="selected"</c:if>>${organization.orgName}</option>
			    				</c:forEach>
			    			</select>
	    				</c:when>
	    				<c:otherwise>
    		...
    		<ul>
	    		<li>
	    			<label>所在区域：
		    			...
		    			<c:if test="${employee.organization != null && employee.organization.parentOrg != null}">${employee.organization.parentOrg.orgName} > </c:if>
		    			<c:if test="${employee.organization != null}">${employee.organization.orgName} </c:if>
		    			<c:if test="${employee.organization == null || employee.organization.orgName == null || employee.organization.orgName == ''}">全国</c:if>
	    			</label>
	    		</li>
	    	</ul>
    	...
    	<ul>
    		<li>
    			<label>用户名：</label>
    			<input type="text"  name="employee.name" id="name" value="${employee.name}"  class="required search-input-text"/>
    		</li>
    	</ul>
    	<ul>
    		<li>
    			<label>联系人姓名：</label>
    			<input type="text"" name="employee.linkman" id="linkman" value="${employee.linkman}" class="required search-input-text">
    		</li>
    	</ul>
    	<ul>
    		<li>
    			<label>办公电话：</label>
    			<input type="text" name="employee.tel" id="tel" value="${employee.tel}" class="required search-input-text">
    		</li>
    	</ul>
    	<ul>
    		<li>
    			<label>手机：</label>
    			<input type="text" name="employee.phone" id="phone" value="${employee.phone}" class="required search-input-text">
    		</li>
    	</ul>
    	<ul>
    		<li>
    			<label>邮箱：</label>
    			<input type="text" name="employee.email" id="email" value="${employee.email}" class="required email search-input-text">
    		</li>
    	</ul>
    	<ul>
    		<li>
    			<label>传真：</label>
    			<input type="text" name="employee.fax" id="fax" value="${employee.fax}" class="required search-input-text">
    		</li>
    	</ul>
    </div>
    </form>
